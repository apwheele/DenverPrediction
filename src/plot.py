...
def auc_plot(data, y_true, y_scores, weight=None, figsize=(6,6), save_plot=False, leg_size=None):
    ...
    for s in y_scores:
        fpr, tpr, thr = roc_curve(data[y_true], data[s], sample_weight=w)
        sub_dat = pd.DataFrame( zip(fpr, tpr, thr), 
                                columns=['FPR','TPR','Thresh'] )
        sub_dat['Var'] = s
        auc_val = roc_auc_score(data[y_true], data[s], sample_weight=w)
        sub_dat['AUC'] = auc_val
...
def auc_plot_long(data, y_true, y_score, group, weight=None, figsize=(6,6), save_plot=False, leg_size=None):
    fin_dat = []
    fig, ax = plt.subplots(figsize=figsize)
    #Equality line
    plt.plot([0, 1], [0, 1], color='grey', lw=1, linestyle='-')
    ...
    for s in grp_list:
        ...
        if weight is not None:
            ...
            w = None
        fpr, tpr, thr = roc_curve(subset[y_true], subset[y_score], sample_weight=w)
        sub_dat = pd.DataFrame( zip(fpr, tpr, thr), 
                                columns=['FPR','TPR','Thresh'] )
        sub_dat['Var'] = s
        auc_val = roc_auc_score(subset[y_true], subset[y_score], sample_weight=w)
        sub_dat['AUC'] = auc_val
        fin_dat.append(sub_dat)
        ax.plot(fpr, tpr, lw=2, label=f'{s} (AUC={auc_val:0.2f})')
    if len(grp_list) == 1:
...
# AUC data plot in wide format, y_scores needs to be a list
# Of the variables in the dataframe
def auc_plot_wide_group(data, y_true, y_scores, group, weight=None, ncols=3,
    ...
    for g,o in zip(grp_list,res_order):
        ...
        for v in y_scores:
            ...
            if weight is not None:
                ...
                w = None
            fpr, tpr, thr = roc_curve(subset[y_true], subset[v], sample_weight=w)
            sub_dat = pd.DataFrame( zip(fpr, tpr, thr), 
                                    columns=['FPR','TPR','Thresh'] )
            sub_dat['Var'] = v
            sub_dat[group] = g
            auc_val = roc_auc_score(subset[y_true], subset[v], sample_weight=w)
            sub_dat['AUC'] = auc_val
            fin_dat.append(sub_dat)
